...
const DEFAULT_SITE = process.env.NEXT_PUBLIC_AGENCY_WEBSITE || "https://thenexusdigital.in";
...
async function loadPersona(): Promise<string> {
  ...
  try {
    ...
    personaCache = "You are Friday, the AI website assistant for Nexus Digital, the leading digital marketing and web development agency in Gorakhpur, India, founded by Sahil Srivastava (Founder, SEO & Full-Stack Web Specialist). You are warm, concise and helpful. Answer using the website content provided when available. Your name is Friday — always introduce yourself as Friday when asked.";
  }
  return personaCache;
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // ── DIRECT CONTACT INTENT (Call / WhatsApp / Message buttons) ──
    const contactIntent =
      /^(call|call us|call now|phone|phone number|contact number|mobile number|give me your number|what is your phone|can i call|call me|whatsapp|send message|send msg|talk to human|speak to human|talk to an agent|connect with human|customer care)[\s!.,?]*$/i.test(message) ||
      /\b(call (button|now|direct|you)|whatsapp (number|link|button|chat)|phone number|talk to a real person|speak with a human|reach by phone)\b/i.test(message);

    if (contactIntent && !isOffTopic(message)) {
      return NextResponse.json({
        reply: `You can reach our senior digital team directly! We are based in Gorakhpur, Uttar Pradesh and available via instant phone call or WhatsApp message:\n\n• Phone: ${CONTACT.phone}\n• WhatsApp: ${CONTACT.phone}\n• Email: ${CONTACT.email}\n\nTap the direct buttons below to connect right away:`,
        action: "contact",
      });
    }
