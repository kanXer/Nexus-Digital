...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-20">
      {/* Header */}
      <div className="relative mb-7 overflow-hidden rounded-3xl bg-[var(--bg-card)] border border-[var(--border-default)] p-7 md:p-8 shadow-card">
        <div className="pointer-events-none absolute -top-24 right-0 w-80 h-80 bg-brand-blue/10 blur-[110px] rounded-full" />
        <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-[var(--text-primary)] leading-tight">Dashboard</h1>
              <p className="text-[var(--text-muted)] text-sm">Live leads at a glance.</p>
            </div>
          </div>
          <button onClick={refreshAll} className="btn-secondary px-4 py-2.5 text-sm self-start sm:self-auto flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-default)] p-5 sm:p-6 lg:col-span-2 shadow-card">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-[var(--text-primary)] font-bold text-base sm:text-lg">Lead Trend</h2>
                <span className="text-[11px] text-[var(--text-muted)]">last 6 months</span>
              </div>
              <LineAreaChart
            ...
            <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-default)] p-5 sm:p-6 shadow-card">
              <h2 className="text-[var(--text-primary)] font-bold text-base sm:text-lg mb-4">Lead Sources</h2>
              <DonutChart
            ...
            <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-default)] p-5 sm:p-6 shadow-card">
              <h2 className="text-[var(--text-primary)] font-bold text-base sm:text-lg mb-4">Leads by Service</h2>
              <HBarList items={overview?.leadByService ?? []} />
            </div>

            {overview && overview.leadMagnets.length > 0 && (
              <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-default)] p-5 sm:p-6 lg:col-span-2 shadow-card">
                <h2 className="text-[var(--text-primary)] font-bold text-base sm:text-lg mb-4">Most Downloaded Lead Magnets</h2>
          ...
          <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-default)] p-5 sm:p-6 shadow-card">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
              <h2 className="text-[var(--text-primary)] font-bold text-base sm:text-lg flex items-center gap-2">
                <Inbox className="w-4 h-4 text-[var(--text-muted)]" /> Recent Submissions
              </h2>
              <div className="flex flex-wrap gap-1.5">
                {tabs.map((t) => (
                  <button
                    ...
                    className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${filter === t.id ? "bg-brand-blue/15 border-brand-blue/40 text-[var(--text-primary)]" : "bg-[var(--bg-secondary)] border-[var(--border-default)] text-[var(--text-muted)] hover:border-[var(--border-hover)] hover:text-[var(--text-secondary)]"}`}
                  >
                    {t.label}
                  </button>
                ))}
            ...
            {loadingList ? (
              ...
              <p className="text-[var(--text-muted)] text-sm text-center py-10">No submissions in this view yet.</p>
            ) : (
              <div className="overflow-x-auto no-scrollbar">
                <table className="w-full text-sm min-w-[640px]">
                  <thead>
                    <tr className="text-[var(--text-muted)] text-xs uppercase tracking-wider text-left border-b border-[var(--border-default)]">
                  ...
                  <tbody className="divide-y divide-[var(--border-default)]">
                    {items.map((it) => (
                      <tr key={it.id} className="text-[var(--text-secondary)] hover:bg-[var(--bg-card-hover)] transition-colors">
                        <td className="py-3 pr-4">
                          <p className="font-medium text-[var(--text-primary)] truncate max-w-[180px]">{String(it.data?.name || it.data?.email || "—")}</p>
                          <p className="text-[var(--text-muted)] text-[11px] truncate max-w-[180px]">{String(it.data?.service || it.data?.resourceTitle || "")}</p>
                        </td>
                        <td className="py-3 pr-4"><span className="text-[10px] font-bold px-2.5 py-1 rounded-full border border-[var(--border-default)] bg-[var(--bg-secondary)] text-[var(--text-secondary)]">{typeLabels[it.type]}</span></td>
                        ...
                        <td className="py-3 pr-4 text-[var(--text-muted)] whitespace-nowrap">{new Date(it.createdAt).toLocaleDateString()}</td>
                        <td className="py-3 pr-4 text-right"><button onClick={() => setSelected(it)} className="text-brand-blue-light hover:underline text-xs font-semibold">View</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-sm" onClick={() => setSelected(null)}>
          <motion.div
            ...
            className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-t-3xl sm:rounded-2xl w-full max-w-lg max-h-[90vh] sm:max-h-[85vh] overflow-y-auto p-5 sm:p-6 shadow-2xl"
          >
            <div className="flex items-start justify-between mb-4">
              <div>
                <span className="text-[10px] font-bold px-2.5 py-1 rounded-full border border-[var(--border-default)] bg-[var(--bg-secondary)] text-[var(--text-secondary)]">{typeLabels[selected.type]}</span>
                <h3 className="text-[var(--text-primary)] font-bold text-lg mt-2">{String(selected.data?.name || selected.data?.email || "Submission")}</h3>
              </div>
              <button onClick={() => setSelected(null)} className="w-8 h-8 rounded-full bg-[var(--bg-secondary)] flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors"><XCircle className="w-4 h-4" /></button>
            </div>
            ...
            <div className="space-y-3 mb-5">
              {Object.entries(selected.data).map(([k, v]) => {
                ...
                return (
                  <div key={k} className="text-sm bg-[var(--bg-secondary)] rounded-xl px-3 py-2.5 border border-[var(--border-default)]">
                    <p className="text-[var(--text-muted)] text-[10px] uppercase tracking-wider mb-0.5">{FIELD_LABELS[k] || k}</p>
                    <p className="text-[var(--text-primary)] break-words font-medium">{val}</p>
                  </div>
                );
              })}
            </div>
            ...
            <div className="flex items-center justify-between gap-3 border-t border-[var(--border-default)] pt-4">
              <button
                ...
                disabled={updating}
                className="inline-flex items-center gap-2 text-sm px-4 py-2.5 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-default)] text-[var(--text-primary)] hover:border-brand-blue/40 disabled:opacity-50 transition-all"
              >
              ...
              <button
                ...
                disabled={deleting}
                className="inline-flex items-center gap-2 text-sm px-4 py-2.5 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 hover:bg-red-500/20 disabled:opacity-50 transition-all"
              >
                {deleting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />} Delete
              </button>
            </div>
            <p className="text-[var(--text-muted)] text-[11px] mt-4">Received {new Date(selected.createdAt).toLocaleString()}</p>
          </motion.div>
        </div>
      )}
    </div>
