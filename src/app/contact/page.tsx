    ...
    <div className="bg-[var(--bg-primary)] text-[var(--text-primary)] min-h-screen transition-colors duration-300">
      <section className="relative pt-32 pb-16 px-4 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto text-center relative z-10">
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2, duration: 0.7, ease: [0.22, 1, 0.36, 1] as const }} className="text-lg text-[var(--text-secondary)] leading-relaxed">
            Tell us about your business and we&apos;ll create a custom growth plan. As the top digital marketing agency in Gorakhpur, Uttar Pradesh businesses count on, Nexus Digital Marketing Agency Gorakhpur, Uttar Pradesh is ready to help your brand rank, advertise, and grow. We offer our services across Gorakhpur, Uttar Pradesh, Lucknow, and all of India.
          </motion.p>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-10">
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] as const }} className="lg:col-span-2 space-y-6">
            <div className="glass-card rounded-2xl p-6 border border-[var(--border-default)]">
              <h3 className="text-[var(--text-primary)] font-bold text-lg mb-5">Get in Touch</h3>
              <div className="space-y-4">
                <a href={`tel:${config.phone}`} className="flex items-center gap-3.5 group">
                  ...
                  <div><p className="text-[var(--text-primary)] font-semibold text-sm">Call Us</p><p className="text-[var(--text-muted)] text-xs">{config.phone}</p></div>
                </a>
                <a href={`mailto:${config.email}`} className="flex items-center gap-3.5 group">
                  ...
                  <div><p className="text-[var(--text-primary)] font-semibold text-sm">Email</p><p className="text-[var(--text-muted)] text-xs">{config.email}</p></div>
                </a>
                <div className="flex items-center gap-3.5">
                  ...
                  <div><p className="text-[var(--text-primary)] font-semibold text-sm">Office</p><p className="text-[var(--text-muted)] text-xs">{config.address}</p></div>
                </div>
              </div>
            </div>
            ...
            <div className="glass-card rounded-2xl overflow-hidden border border-[var(--border-default)] h-48">
              <iframe
                src={`https://www.google.com/maps?q=Nexus+Digital+Marketing+Agency,+Gorakhpur,+Uttar+Pradesh&output=embed`}
                width="100%"
                height="100%"
                ...
                title="Nexus Digital Office Location - Gorakhpur, Uttar Pradesh"
              />
            </div>
            <a
              ...
              className="flex items-center justify-center gap-2 text-xs text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors mt-1"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 mt-2 w-full py-3 rounded-xl text-sm font-semibold text-[var(--text-primary)] bg-brand-blue/10 border border-brand-blue/25 hover:bg-brand-blue/20 transition-all group"
            >
              <Star className="w-4 h-4 text-amber-400 fill-amber-400" />
              View our Google Business Profile
              <span className="text-xs text-[var(--text-muted)] group-hover:text-[var(--text-secondary)]">({config.gmbRating}/5 Rating · {config.gmbReviewCount}+ Google Reviews)</span>
            </a>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] as const }} className="lg:col-span-3">
            <div className="glass-card rounded-3xl p-6 md:p-8 border border-[var(--border-default)]">
              {!submitted ? (
                <>
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <h3 className="text-[var(--text-primary)] font-bold text-xl">Need Help? We&apos;re Here for You</h3>
                  </div>
                  <p className="text-[var(--text-secondary)] text-sm mb-4">Koi bhi problem, sawaal ya help chahiye? Form bharo — hum 24 hours ke andar reply karenge. Best digital marketing agency in Gorakhpur, Uttar Pradesh and India ki complete services ke liye contact karein.</p>
                  <div className="flex items-center gap-2 bg-green-500/10 border border-green-500/20 px-3 py-2 rounded-lg mb-6">
                    ...
                    <p className="text-xs text-green-600 dark:text-green-400 font-semibold">⚡ Fast Response: We usually reply within 5 minutes on WhatsApp.</p>
                  </div>
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="text-[var(--text-secondary)] text-xs font-medium mb-1.5 block">Full Name *</label>
                        <input type="text" name="name" value={formData.name} onChange={handleChange} required placeholder="John Doe" className="input-field" />
                      </div>
                      <div>
                        <label className="text-[var(--text-secondary)] text-xs font-medium mb-1.5 block">Email Address *</label>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="text-[var(--text-secondary)] text-xs font-medium mb-1.5 block">Phone Number *</label>
                      ...
                      <div>
                        <label className="text-[var(--text-secondary)] text-xs font-medium mb-1.5 block">Business Name</label>
                    ...
                    <div>
                      <label className="text-[var(--text-secondary)] text-xs font-medium mb-1.5 block">Service Required *</label>
                    ...
                    <div>
                      <label className="text-[var(--text-secondary)] text-xs font-medium mb-1.5 block">Monthly Budget Range</label>
                    ...
                    <div>
                      <label className="text-[var(--text-secondary)] text-xs font-medium mb-1.5 block">How Can We Help You? *</label>
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-12">
                  ...
                  <p className="text-[var(--text-secondary)] text-sm">Thank you for reaching out. Our team will review your details and get back to you within 24 hours.</p>
                </motion.div>
              )}
            </div>
          </motion.div>
      ...
      <section id="book-consultation" className="px-4 sm:px-6 lg:px-8 pb-24 relative overflow-hidden scroll-mt-24">
        ...
        <div className="max-w-3xl mx-auto relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] as const }} className="glass-card rounded-3xl p-6 md:p-10 border border-[var(--border-default)]">
            {!bookingSubmitted ? (
              <>
                ...
                <p className="text-[var(--text-secondary)] text-sm mb-4">Pick a date &amp; time that works for you. The team at our digital marketing company in Gorakhpur, Uttar Pradesh will join the call with a custom growth plan — 100% free.</p>
                ...
                <form onSubmit={handleBookingSubmit} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="text-[var(--text-secondary)] text-xs font-medium mb-1.5 block">Preferred Date *</label>
                      <input type="date" name="date" value={bookingData.date} onChange={handleBookingChange} required className="input-field" />
                    </div>
                    <div>
                      <label className="text-[var(--text-secondary)] text-xs font-medium mb-1.5 block">Preferred Time *</label>
                  ...
                  <div>
                    <label className="text-[var(--text-secondary)] text-xs font-medium mb-1.5 block">Anything you&apos;d like to discuss?</label>
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-12">
                <div className="w-16 h-16 rounded-2xl bg-green-500/20 border border-green-500/30 flex items-center justify-center mx-auto mb-5"><CheckCircle className="w-8 h-8 text-green-400" /></div>
                <h3 className="text-2xl font-bold text-[var(--text-primary)] mb-2">Consultation Booked!</h3>
                <p className="text-[var(--text-secondary)] text-sm">Thank you! Our team will confirm your meeting slot via email within 24 hours. We can&apos;t wait to grow your business.</p>
              </motion.div>
            )}
          </motion.div>
        </div>
