...
function CheckoutContent() {
  ...
  // Paytm runs in demo/simulation mode until real keys are added to .env
  // (NEXT_PUBLIC_PAYTM_LIVE=true).
  const paytmLive = process.env.NEXT_PUBLIC_PAYTM_LIVE === "true";
  ...
  return (
    <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8">
      ...
      <div className="md:col-span-5 space-y-6">
        {billingComplete ? (
          <div className="paytm-checkout-card always-dark relative overflow-hidden rounded-3xl p-6 border space-y-5 bg-gradient-to-br from-[#062a3a] via-[#08202e] to-[#0b0b0d] border-[#00baf2]/30 shadow-[0_0_40px_rgba(0,186,242,0.15)]">
            {/* Ambient glow */}
            <div className="absolute -top-20 left-1/2 -translate-x-1/2 w-72 h-36 bg-[#00baf2]/15 rounded-full blur-[90px] pointer-events-none" />
            ...
            {!paytmLive && (
              <div className="relative flex items-start gap-2 p-3 rounded-xl bg-amber-500/10 border border-amber-500/30">
                ...
                <p className="text-[11px] text-amber-200/80 leading-relaxed">
                  <strong className="text-amber-300">Demo / Test Mode:</strong> Paytm keys are not configured in <code>.env</code>. Clicking pay will simulate a successful payment (no real charge). Add your keys &amp; set <code>NEXT_PUBLIC_PAYTM_LIVE=true</code> for live payments.
                </p>
              </div>
            )}
            ...
            <div className="relative">
              <h4 className="font-bold text-white text-lg flex items-center gap-2 mb-1">
                <CreditCard className="w-5 h-5 text-[#7fe3ff]" /> Express Checkout
              </h4>
              <p className="text-white/50 text-xs">
                Secure SSL Encryption. Instant Order Activation via Paytm.
              </p>
            </div>
          ...
          <div className="glass-card rounded-3xl p-8 border border-white/10 text-center space-y-4">
            ...
            <p className="text-white/55 text-sm">
              Complete your billing address on the left to unlock secure Paytm payment options.
            </p>
            <button
              onClick={openProfileModal}
